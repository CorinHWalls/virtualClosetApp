...
import { getCategoryItems, getAllItems, getItemById } from "../Services/ItemService";

export default function CreateLookScreen({navigation}) {
  const { currentUser, setSelectedItemId} =
  ...
  const handleLongPress = (item) => {
   ...
   const newItemList =  selectedItems.filter(itemId => itemId !== item.id)
     //returning setSelectedItems so the code underneeth does not run
  ...
  return (
    <>
      <SafeAreaView style={styles.container}>
        ...
        <Box mt="5" borderWidth="1" w="100%" h="55%">
              ...
              return (
                <>
                  <TouchableOpacity
                    ...
                    // selected={}
                    onLongPress={() => {handleLongPress(item), console.log(getSelected(item))}}
                    key={index}
                  >
